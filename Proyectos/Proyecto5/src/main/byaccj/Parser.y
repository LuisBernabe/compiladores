...
;
/*    aux2: (and_test 'or')+  */
aux2: not_test OR {$$ = new OrNodo($1, null);}
    | aux2 not_test OR {$$ = $1; 
        $3.agregaHijoPrincipio($2); $$.agregaHijoFinal($3);}
;
...
aux9: factor POR {$$ = new MultNodo($1,null);}
    ...
    | aux9 factor POR {$1.agregaHijoFinal($2);
        $$ = new MultNodo($1,null);}
    | aux9 factor DIVENTERA {$1.agregaHijoFinal($2);
        $$ = new DivENodo($1,null);}
    | aux9 factor MODULO {$1.agregaHijoFinal($2); 
        $$ = new ModNodo($1,null);}
    | aux9 factor DIV {$1.agregaHijoFinal($2); 
        $$ = new DivNodo($1,null);}
;
/* factor: ('+'|'-') factor | power */
